...
- **本益比 / 殖利率 / 股價淨值比**（來源：證交所 `BWIBBU_ALL`）
- **三大法人買賣超**（外資／投信／自營商，來源：證交所 `T86`）
- **融資融券餘額**（來源：證交所 `MI_MARGN`）

這三份都是證交所的官方免費公開資料，一樣由 `scripts/fetch_tw_quotes.py` 跟股價
一起抓、每 30 分鐘更新一次。表格裡的欄位名稱**直接使用證交所報表原本的中文
標題**（例如「三大法人買賣超股數」），沒有另外翻譯或改名，確保意思跟證交所
公告的定義完全一致。

美股沒有對應的免費資料來源，所以這三個表格只會在你選台股的時候出現。

**如果某個表格一直顯示「查無資料」：** 有可能是當天證交所還沒公布（例如盤中
還沒收盤），也可能是證交所調整了報表格式，導致程式抓不到股號欄位去比對。
發生這種狀況可以看 GitHub Actions 該次執行紀錄裡有沒有印出 ⚠️ 開頭的警告
訊息，把訊息內容告訴我，我可以幫忙調整程式對應新的欄位名稱。
